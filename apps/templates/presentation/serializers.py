...
class TemplateItemSerializer(serializers.Serializer):
    ...
    source_row = serializers.IntegerField(min_value=1)
...
class TemplateCreateUpdateSerializer(serializers.Serializer):
    code = serializers.ChoiceField(choices=[TemplateCode.MOTOS, TemplateCode.LIVIANOS_PESADOS])
    name = serializers.CharField(validators=[RegexValidator(regex='^(?![0-9]*$)[a-zA-Z0-9\s]+$',message='El nombre debe contener letras y no puede ser solo numérico.')])
    ...
    source_file = serializers.CharField(required=False, allow_blank=True, default='')
