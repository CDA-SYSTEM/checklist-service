"""
MongoEngine Documents para el bounded context Inspections.

Modelos de persistencia — no son entidades de dominio.
Incluye los EmbeddedDocuments de labrado porque se persisten dentro de Inspection.
"""
...
class Inspection(Document):
    ...
    vehicle_id = IntField(required=True)
    client_id = IntField()
    ...
    inspector_id = StringField(required=True)
    status = StringField(
        required=True,
        choices=InspectionStatus.ALL,
        default=InspectionStatus.BORRADOR,
    )
    responses = ListField(
        EmbeddedDocumentField(InspectionItemResponse), default=list
    )
